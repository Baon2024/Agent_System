...
class Agent:
    ...
    def __init__(self, passed_system_prompt, name, state: CustomState, tools: list = None, num_cycles: int = 30, sub_agents: list = None, mcp_urls: list = None):
        self.system_prompt = []
    ...
    def get_current_agent_messages(self, messages, agent_id):
        ...
        filtered_messages = [
            {k: v for k, v in message.items() if k != "agent_id" and k != "message_id" and k != "session_id"}
    ...
    async def get_next_step(self, messages, agent_id): ## rename to execute?
        ...
        formatted_answer = formatted_answer.model_dump()
        print(f"formatted_answer is: {formatted_answer}")
        messages.append({"role": "assistant", "content": json.dumps(formatted_answer), "agent_id": agent_id, "session_id": self.session_id })
        return formatted_answer
    ...
    async def run(self, messages_memory, agent_id, current_ws):
                    ...
                    if action["type"] == "function":
                        ...
                        if function in self.known_mcp_tools_for_agent.keys():
                            ...
                            if args:
                                ...
                                function_result = self.known_tools_for_agent[function](self.state)
            ...
            ## check files agent is returning, for each file add to supabase bucket, then create presignedurl
                ...
                if final_answer.get('files'):
                    ...
                    for file in files:
                        ...
                        storage_path = await save_file_to_supabase(file_name)
...
async def get_agent():
    ...
    # if agent already exists, return form cach
    if _cached_agent is not None:
        print("_cached_agent already exists - returning it...")
        return _cached_agent
    
    async with _agent_lock:
        if _cached_agent is not None:
            print("inside of _agent_lock: _cached_agent already exists - returning it...")
            return _cached_agent
        
        print("_cached_agent does not exist - creating it...")
        _cached_agent = Agent(system_prompt_programatic_ollama, "Main Agent", ollama_state, tools=[firecrawl_search, run_python, how_to_web_search, edit_todo_list, write_todo_list, read_todo_list, read_file, multiple_concurrent_sub_agents, write_file, read_folder_files, ask_user_followup, read_skill], mcp_urls=["http://127.0.0.1:8000/mcp"])
        print("_cached_agent created - running get_mcp_tools...")
        await _cached_agent.get_mcp_tools()
        print("_cached_agent has fetched get_mcp_tools -  returning _cached_agent...")
        return _cached_agent
     
     #async def __init__(self, system_prompt, name, state: CustomState, tools: list = None, num_cycles: int = 30, sub_agents: list = None, mcpUrls: list = None):
    #print(f"result from multi-agent versions is: {result}")
    #print(f"total tokens_used: {tokens_used}")

    #with open("conversation_messages1.txt", "w", encoding="utf-8") as file:
        #json.dump(conversation_messages, file)
